...
######################## USER INPUT STARTS HERE ######################
    ...
    # Specify any given information for each event (a vector of 1s 
    # means there is no information given for that event.)
    ...
    # N: [1,0]/[0,1] means we HAVE information (it's an evidence node)
...
######################### USER INPUT ENDS HERE #######################
    ...
    # Set up structures to store parent and child information 
    #   for each node
    parent={}
    children={}
    count={}
    ...
    while len(Q)!=0:
        ...
        # if the node is not the root node, send information to its parent node
        if i in parent: 
    ...
    while len(Q)!=0:
        i=Q.pop(0)
        # if the node is the root node, pi is set to be the 
        # prior distribution at the node
        if i not in parent: 
            pi[i]=M[i].T
        # otherwise, pi is the matrix product of the message from 
        # the parent and the conditional probability at the node
        else: 
